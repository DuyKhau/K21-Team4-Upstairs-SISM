...
namespace K21_Team4_Upstairs_SISM.Controllers
{
	public class GroupController : Controller
	{
		...
		[HttpGet]
		public ActionResult AddNewStudentAsync()
		{
			return View();
		}
		[HttpPost]
		//public async System.Threading.Tasks.Task<ActionResult> AddNewStudentAsync(string id)
		//{
		//	var userEmail = id;
		//	var userid = db.AspNetUsers.FirstOrDefault(x => x.Email == userEmail);
		//	if (userid == null)
		//	{
		//		await InsertUserAsync(userEmail);
		//	}

		//	var uID = db.AspNetUsers.FirstOrDefault(x => x.Email == userEmail).Id;
		//	if (db.Members.FirstOrDefault(x => x.UserID == uID) == null)
		//	{
		...
		//		db.SaveChanges();
		//	}
		//	var member = db.Members.FirstOrDefault(x => x.UserID == uID);
		...
		private static List<User> ConvertDataTable<T>(DataTable dt)
		{
			...
			foreach (DataRow row in list)
			{
				...
				us.DoB = DateTime.Parse(row[4].ToString());
